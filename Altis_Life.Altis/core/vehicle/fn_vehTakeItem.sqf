...
if(isNull life_trunk_vehicle OR !alive life_trunk_vehicle) exitWith {hintSilent localize "STR_MISC_VehDoesntExist"};
if(!alive player) exitwith {closeDialog 0;};

if((lbCurSel 3502) == -1) exitWith {hintSilent localize "STR_Global_NoSelection";};
_ctrl = ctrlSelData(3502);
_num = ctrlText 3505;
if(!([_num] call TON_fnc_isnumber)) exitWith {hintSilent localize "STR_MISC_WrongNumFormat";};
_num = parseNumber(_num);
if(_num < 1) exitWith {hintSilent localize "STR_MISC_Under1";};
