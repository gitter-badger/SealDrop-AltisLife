...
if(life_inv_kidney >= 2) exitWith {hintSilent "You can't possibly need that many organs!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hintSilent "You don't have room to store any organs!"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
