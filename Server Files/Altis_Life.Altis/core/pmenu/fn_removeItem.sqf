...
if(_data == "") exitWith {hintSilent "You didn't select anything to remove.";};
if(!([_value] call TON_fnc_isnumber)) exitWith {hintSilent "You didn't enter an actual number format."};
if(parseNumber(_value) <= 0) exitWith {hintSilent "You need to enter an actual amount you want to remove."};
...
if(!([false,_data,(parseNumber _value)] call life_fnc_handleInv)) exitWith {hintSilent "Couldn't remove that much of that item, maybe you don't have that amount?"};
