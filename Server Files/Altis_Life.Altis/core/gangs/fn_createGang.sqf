...
if(_length > 32) exitWith {hintSilent localize "STR_GNOTF_Over32"};
_badChar = false;
{if(!(_x in _allowed)) exitWith {_badChar = true;};} foreach _chrByte;
if(_badChar) exitWith {hintSilent localize "STR_GNOTF_IncorrectChar";};
if(life_atmcash < (__GETC__(life_gangPrice))) exitWith {hintSilent format[localize "STR_GNOTF_NotEnoughMoney",[((__GETC__(life_gangPrice))-life_atmcash)] call life_fnc_numberText];};

[[player,getPlayerUID player,_gangName],"TON_fnc_insertGang",false,false] spawn life_fnc_MP;
hintSilent localize "STR_NOTF_SendingData";
closeDialog 0;
